...
def displaygraphs(measured_csv_file, start_display, end_display, title_label):
    ...
    with open(measured_csv_file, 'r') as csvfile:
        ...
        for row in datareader:
            if row == []:
                ...
                if dt<start_display or dt>end_display:
...
def main():
    ...
    for files in meas_list:
        displaygraphs(files, first_recording, last_recording, str(first_recording) +" - "+ str(last_recording))
        #Graph each data collection unit on a separate graph for Day 1
    first_recording = datetime.datetime.strptime("11/17/2016 10:00:00", "%m/%d/%Y %H:%M:%S")
    last_recording = datetime.datetime.strptime("11/17/2016 14:30:00", "%m/%d/%Y %H:%M:%S")
    for files in meas_list:
        displaygraphs(files, first_recording, last_recording, str(first_recording) +" - "+str(last_recording))
    
        #Graph each data collection unit on a separate graph for Day 2
    first_recording = datetime.datetime.strptime("11/18/2016 09:00:00", "%m/%d/%Y %H:%M:%S")
    last_recording = datetime.datetime.strptime("11/18/2016 14:30:00", "%m/%d/%Y %H:%M:%S")
    for files in meas_list:
        displaygraphs(files, first_recording, last_recording, str(first_recording) +" - "+str(last_recording))
